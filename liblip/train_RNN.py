...
if __name__ == '__main__':
    ...
    label_train_reduced = np.load('../data/label_train_reduced.npy')
    features_train = np.load('../data/features_training.npy')
    label_test_reduced = np.load('../data/label_test_reduced.npy')
    ...
    out_dim = label_test_reduced.shape[1]
    hidden_levels = 1
    lstm_cells = 250
    ...
    # model.summary()
    model.compile(optimizer=sgd, loss='categorical_crossentropy', metrics=['accuracy'])   # TODO: use CTC loss function

    # Train
    print('Training Net')
    model.fit(features_train, label_train_reduced, batch_size=100, epochs=1, callbacks=checkpoint,
              validation_data=(features_test, label_test_reduced))
